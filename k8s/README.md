...
1. Access to a k8s cluster. _The instructions in this file have been verified with a local k8s cluster running via Docker Desktop._
...
Verify if everything is running by running `kubectl get pods`. _Ignore if `prometheus node-exporter` pod fails_.
...
Run the following commands to uninstall everything
